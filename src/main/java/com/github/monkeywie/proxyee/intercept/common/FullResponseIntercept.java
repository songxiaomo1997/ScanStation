...
public abstract class FullResponseIntercept extends HttpProxyIntercept {
    ...
    public final void afterResponse(Channel clientChannel, Channel proxyChannel,
        ...
        if (httpResponse instanceof FullHttpResponse) {
            ...
            // 判断后续的拦截器是否匹配
            boolean isAfterMatch = isFirstMatch ? false : match(clientChannel,pipeline.getHttpRequest(), pipeline.getHttpResponse(), pipeline);
            ...
            this.isMatch = match(clientChannel,pipeline.getHttpRequest(), pipeline.getHttpResponse(), pipeline);
            if (this.isMatch) {
     ...
     */
    public abstract boolean match(Channel clientChannel,HttpRequest httpRequest, HttpResponse httpResponse,
